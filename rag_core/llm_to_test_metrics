...
from metrics.ariscore_eval import main as ari_main
from metrics.bertscore_eval import main as bertscore_main
from metrics.ragscore_eval import main as rag_score_main

def run_all_metrics():
    print("\n--- ARI SCORE ---")
    ari_main(
        input_filename='pgverse/data/input1/data1.txt',
        direct_filename='direct_output.txt',
        iterative_filename='iterative_output.txt',
    )
    print("\n--- BERTSCORE ---")
    bertscore_main(
        input_file='wyniki_bert.txt',
...
def save_chunks_to_neo4j(file_path, api_key, neo4j_uri, neo4j_user, neo4j_password, max_tokens=500):
    ...
    graph_builder.close()
    # Zapisz segmenty do pliku direct_output.txt (lub innego, zależnie od metody)
    with open("direct_output.txt", "w", encoding="utf-8") as f:
        for idx, chunk in enumerate(chunks, 1):
            f.write(f"Segment {idx}\n{chunk}\n\n")  # lub po prostu f.write(chunk + "\n\n") jeśli nie chcesz nagłówków
            
    iterative_chunks = chunker.chunk_from_file(file_path, max_tokens=250)
    with open("iterative_output.txt", "w", encoding="utf-8") as f:
        for idx, chunk in enumerate(iterative_chunks, 1):
            f.write(f"Segment {idx}\n{chunk}\n\n")
    driver.close()
...
if __name__ == "__main__":
    ...
    while True:
        ...
        # Zapisz odpowiedź modelu dla RAG Alignment Score
        with open("generated.txt", "w", encoding="utf-8") as f:
            f.write(response_text)

        # Zapisz retrieved chunki dla RAG Alignment Score
        with open("retrieved.txt", "w", encoding="utf-8") as f:
            for src in sources:
                f.write(src.replace('\n', ' ') + '\n')

        # Zapisz log do BERTScore (możesz też zrobić to na końcu, jeśli masz wiele pytań)
        with open("wyniki_bert.txt", "a", encoding="utf-8") as f:
            f.write(f"Pytanie: {query}\n\nOdpowiedź modelu:\n{response_text}\n\nZnalezione chunki tekstu:\n")
            for idx, src in enumerate(sources, 1):
                f.write(f"Chunk {idx} (score: brak):\n{src}\n\n")
            f.write("-" * 80 + "\n")
