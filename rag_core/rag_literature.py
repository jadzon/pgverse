...
def process_query(query, file_path, cohere_api_key, neo4j_uri, neo4j_username, neo4j_password):
    ...
    response = co.chat(
        message=f"Odpowiedz na pytanie używając tylko informacji z podanego kontekstu, w języku polskim: {query}",
        preamble="Jesteś asystentem, który odpowiada na pytania dotyczące 'Opowieści wigilijnej' Charles'a Dickensa w języku polskim.",
        documents=[{"snippet": chunk["text"]} for chunk in reranked_chunks[:3]]
    )
...
if __name__ == "__main__":
    file_path = "opowiesc.txt"
    cohere_api_key = "xSywHzTHlEcq51tOI8rpxwWwtDdQnio5H7pPnuxs"
    neo4j_uri = "neo4j+s://dafc68b2.databases.neo4j.io"
    neo4j_username = "neo4j"
    neo4j_password = "JKHIqwOLFS9nzMqd0_wSniLKXq-HxQddCooXBfO9KGQ"

    queries = [
       "Co przedstawiały obrazy na kaflach pieca w domu Ebenezera?"
    ]
    ...
    for query in queries:
        result = process_query(query, file_path, cohere_api_key, neo4j_uri, neo4j_username, neo4j_password)
        results.append(result)
        metrics_list.append(result['metrics'])

        print(f"Pytanie: {result['query']}")
